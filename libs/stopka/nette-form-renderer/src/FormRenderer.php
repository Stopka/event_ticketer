...
class FormRenderer implements Nette\Forms\IFormRenderer {
    ...
    /**
     * Vrátí vykreslenou část formuláře
     * @param string $mode
     * @return string
     */
    public function renderMode($mode) {
        ...
        switch (strtolower($mode)) {
            ...
            default:
                throw new Nette\Application\ApplicationException("Unsupported form renderer mode '$mode'");
        }
    }
...
//    }
        ...
        if ($embed_group_names = $group->getOption('embed')) {
            ...
            foreach ($embed_group_names as $embed_group_name){
                ...
                if ($embed_group == $group) {
                    throw new Nette\Application\ApplicationException('Group can\'t embed itself');
                }
     ...
     * @return string
     */
    public function renderGroupControls($parent) {
